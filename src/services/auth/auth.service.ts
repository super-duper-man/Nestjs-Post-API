...
export class AuthService {
    constructor(@InjectRepository(UserEntity) private readonly userRepo: Repository<UserEntity>, private jwtService: JwtService) { }

    async register(dto: RegisterDto) {
        const exitsUser = await this.userRepo.findOne({ where: { email: dto.email } });

        if (!exitsUser)
            throw new ConflictException('کاربری با این ایمیل آدرس قبلا ساخته شده است');
    ...
    private async verifyPassword(password: string, dbPass: string): Promise<boolean> {
